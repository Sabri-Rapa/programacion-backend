...
const { DB_MONGOATLAS_USER, DB_MONGOATLAS_PASS, SESSION_SECRET } = process.env;
...
const args = yargs
 ...
 .argv
 const PORT = process.argv.slice(2).toString() || args.PORT || 8080
  ...
  new LocalStrategy(
    ...
    (req, username, password, done) => {
      Users.findOne({ username: username }, function (error, user) {
        ...
        const { nombre, apellido, email, edad, direccion, telefono } = req.body;
        const newUser = {
          nombre,
          apellido,
          email,
...
app.use(
  session({
    store: MongoStore.create({
      mongoUrl: `mongodb+srv://${DB_MONGOATLAS_USER}:${DB_MONGOATLAS_PASS}@cluster0.3wmnfr7.mongodb.net/?retryWrites=true&w=majority`,
      mongoOptions: {
